...
/****************************************************/
	...
	function header(){
		...
		if(!$user_ID && !$this->allow_unreg()){
			$meta = "<p>".__("Welcome Guest, posting in this forum requires BFI Membership", "mingleforum").$this->login_form();
			$colspan = "";
		}
		$o = "<div class='wpf'>
...
//					__("Email:", "mingleforum")." ".$submitter_email."<br/>".
	...
	function notify_admins($thread_id, $subject, $content, $date)
	{
		...
		if(!empty($to)) {
			...
			wp_mail($to, $subject, $custom_content, $headers);
		}
	}
	...
	function custom_output($content,$html_entity_decode,$auto_embed,$do_shortcode) 
	{
		$custom_content = $content;
		$custom_content = $html_entity_decode?html_entity_decode($custom_content):$custom_content;
