...
<div id="demo-source">
	...
	<div id="demo-panel">
		...
		<div class="related">	
			...
			<div alt="s1" class="tab-style ts1"></div>
...
jQuery(document).ready(function(d) {
	...
	//var selected-style = c_value;
	if (!c_value) c_value = 's1';
	d("div[alt='" + c_value +"']").addClass('tab-style-current');
		...
		d('.tab-style').click(function(){
				...
				document.cookie="bfi_selected_style="+escape(s)+";expires="+exdate.toUTCString()+";path=/";
				if (s == 's1') {
					d('link[id="predetermined-css"]').attr('disabled', 'disabled');
					d('link[id="predetermined-css"]').remove();
				}
				else {
					var css = "<link rel='stylesheet' id='predetermined-css' href='<?php echo $template_url ?>/styles/predetermined/" + s + ".css' type='text/css' media='all' />";
					// set new demo-style
					d('head').append(css);
				}
...
.ts5 { background:url(<?php echo $template_url ?>/admin/graphics/styles/for-demo-widget.png) -100px top no-repeat; }
.ts6 { background:url(<?php echo $template_url ?>/admin/graphics/styles/for-demo-widget.png) -120px top no-repeat; }
.ts7 { background:url(<?php echo $template_url ?>/admin/graphics/styles/7.png) top no-repeat; background-size:contain; }
