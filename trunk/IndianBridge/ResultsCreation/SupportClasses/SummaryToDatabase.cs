...
{
    public  class SummaryToDatabase
    {
        private EventInformation m_eventInformation;
        private DatabaseParameters m_databaseParameters;

        public DatabaseParameters getDatabaseParameters() { return m_databaseParameters; }

        public SummaryToDatabase(EventInformation eventInformation)
        {
        ...
        {
            m_databaseParameters = General.createDefaultDatabaseParameters();
            System.IO.File.Delete(m_eventInformation.databaseFileName);
            String sourceFileName = System.IO.Path.Combine(System.IO.Directory.GetCurrentDirectory(),"Databases", "ScoreDatabaseTemplate.mdb");
            ...
            doRanking_();
            // Section Ranking
            if (m_eventInformation.hasDirectionField) doRanking_(General.SINGLE_SECTION_NAME, "Session_Rank");
            else
            {
                doRanking_(General.NORTH_SOUTH_SECTION_NAME, "Session_Rank");
                doRanking_(General.EAST_WEST_SECTION_NAME, "Session_Rank");
            }
        ...
        private  int getPairInformation_(String[] lines, int lineNumber, out String sectionName, out int pairNumber, bool update = false)
        {
            ...
            if (tokens.Length > 1)
            {
                ...
                sectionName = (Utilities.containsPattern_(text, General.NORTH_SOUTH_SECTION_NAME) ? General.NORTH_SOUTH_SECTION_NAME : (Utilities.containsPattern_(text, General.EAST_WEST_SECTION_NAME) ? General.EAST_WEST_SECTION_NAME : General.SINGLE_SECTION_NAME));
            ...
            String result, String score, String datum, bool update = false)
        {
            ...
            else
            {
                Object[] keys = new Object[3];
                keys[0] = boardNumber;
                keys[1] = General.findOtherSectionName(sectionName);
                keys[2] = opponent;
                DataRow dRow = m_databaseParameters.m_ds.Tables["Board_Wise_Scores"].Rows.Find(keys);
                if (dRow == null) return;
            ...
            int opponent, String result, String score, String datum, bool update = false)
        {
            ...
            {
                Object[] keys = new Object[3];
                keys[0] = General.findOtherSectionName(sectionName);
                keys[1] = opponent;
                keys[2] = boardNumber;
                DataRow dRow = m_databaseParameters.m_ds.Tables["Pair_Wise_Scores"].Rows.Find(keys);
                if (dRow == null) return;
