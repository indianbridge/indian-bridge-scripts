var previous_results_page_tab = null;
function switchResultsTab(newTab, tabIDPrefix, server_side_url, member_id,db_name) {
	var selectedClass = 'selected';
	if (previous_masterpoint_page_tab != null)
		jQuery("#" + tabIDPrefix + previous_results_page_tab).removeClass(selectedClass);
	jQuery("#" + tabIDPrefix + newTab).addClass(selectedClass);
	previous_results_page_tab = newTab;
	bfi_results_renderTable_dataTables(newTab, server_side_url, member_id,db_name);
}

function bfi_results_renderTable_dataTables(tableType, server_side_url, member_id, db_name) {
	var html = '<table id="bfi_masterpoints_table"><thead><tr>';
	if (tableType == "leaderboard") {
		var columns = new Array("Round", "Opp_Number", "Opp_Name", "Score_1", "Score_2");
		var sortColumn = "Round";
		var SortOrder = "asc";
		...
		var columns = new Array("Team Number", "Team Name","Member Names","Total");
		var sortColumn = "Team Number";
		var SortOrder = "asc";		
	}
	else {
