...
{
	print "<div> Please configure the Plugin by <a href='admin.php?page=wpfileman'> Visiting Here </a>";
}
#else
#{
...
print "<table class='index' cellpadding=0 cellspacing=0>";
 print "<tr>";
  print "<td class='iheadline' colspan=4 align='center' height=21>";
   print "<font class='iheadline'>$StrIndexOf&nbsp;".get_linked_path($wp_fileman_path,$base_url)."</font>";
  print "</td>";
 print "</tr>";
 print "<tr>";
  print "<td>&nbsp;</td>";
  ...
  if ($open = @opendir($home_directory.$wp_fileman_path))
  {
   for($i=0;($directory = readdir($open)) != FALSE;$i++)
    if (is_dir($home_directory.$wp_fileman_path.$directory) && $directory != "." && $directory != ".." && !is_hidden_directory($home_directory.$wp_fileman_path.$directory))
  ...
  if ($open = @opendir($home_directory.$wp_fileman_path))
  {
   for($i=0;($file = readdir($open)) != FALSE;$i++)
    if (is_file($home_directory.$wp_fileman_path.$file) && !is_hidden_file($home_directory.$wp_fileman_path.$file))
    {
     $icon = get_icon($file);
  ...
  print "<table class='files' width=500 cellpadding=1 cellspacing=0>";
   print "<tr>";
    print "<td class='bold' width=20>&nbsp;</td>";
    print "<td class='bold'>&nbsp;<a href='$base_url&amp;path=".htmlentities(rawurlencode($wp_fileman_path))."&amp;sortby=filename&amp;order=".get_opposite_order("filename", $_GET['order'])."'>$StrFileNameShort</a></td>";
    print "<td class='bold' width=60 align='center'><a href='$base_url&amp;path=".htmlentities(rawurlencode($wp_fileman_path))."&amp;sortby=filesize&amp;order=".get_opposite_order("filesize", $_GET['order'])."'>$StrFileSizeShort</a></td>";
...
// echo get_option('siteurl'); /wp-admin/admin-ajax.php?action=choice&width=150&height=100"  title="Choice"
    if ($AllowEdit && is_editable_file($file['filename'])) print "<td width=20><a href='$base_url&amp;path=".htmlentities(rawurlencode($wp_fileman_path))."&amp;filename=".htmlentities(rawurlencode($file['filename']))."&amp;action=edit'><img src='" . WP_CONTENT_URL . "/plugins/wp-filemanager/icon/edit.gif' width=20 height=22 alt='$StrEditFile' border=0></a></td>";
