...
{
	unset($wp_fileman_path);
}
if (isset($_GET['path']))
    $wp_fileman_path = validate_path($_GET['path']);
else if (isset($_POST['path']))
    $wp_fileman_path = validate_path($_POST['path']);
/*else
	if ( function_exists('get_option') && get_option('wp_fileman_home') != '')
	{
		$wp_fileman_path = get_option('wp_fileman_home');
	}
...
if (isset($_SESSION['session_username']) && $_SESSION['session_username'] == $username && isset($_SESSION['session_password']) && $_SESSION['session_password'] == md5($password) || !$phpfm_auth)
{
    ...
    if (isset($_GET['action']) && is_file(WP_CONTENT_DIR . "/plugins/wp-filemanager/incl/".$_GET['action'].".inc.php") && is_valid_name($_GET['action']))
	{
        include(WP_CONTENT_DIR . "/plugins/wp-filemanager/incl/".basename($_GET['action']).".inc.php");
	}
    else if (isset($_GET['output']) && is_file(WP_CONTENT_DIR . "/plugins/wp-filemanager/incl/".$_GET['output'].".inc.php") && is_valid_name($_GET['output']))
    {
        print "<table class='output' width=400 cellpadding=0 cellspacing=0>";
            print "<tr><td align='center'>";
